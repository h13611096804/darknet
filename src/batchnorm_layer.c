...
void forward_batchnorm_layer(layer l, network_state state)
{
	/*if (l.xnor) {
	if (state.train) {
	mean_cpu(state.input, l.batch, l.c, l.h*l.w, l.mean);
	variance_cpu(state.input, l.mean, l.batch, l.c, l.h*l.w, l.variance);

	scal_cpu(l.c, .9, l.rolling_mean, 1);
	axpy_cpu(l.c, .1, l.mean, 1, l.rolling_mean, 1);
	scal_cpu(l.c, .9, l.rolling_variance, 1);
	axpy_cpu(l.c, .1, l.variance, 1, l.rolling_variance, 1);

	copy_cpu(l.outputs*l.batch, l.output, 1, l.x, 1);
	normalize_cpu(state.input, l.mean, l.variance, l.batch, l.c, l.h*l.w);
	copy_cpu(l.outputs*l.batch, l.output, 1, l.x_norm, 1);
	}
	else {
	normalize_cpu(state.input, l.rolling_mean, l.rolling_variance, l.batch, l.c, l.h*l.w);
	}
	scale_bias(state.input, l.scales, l.batch, l.c, l.h*l.w);
	return;
	}*/
...
{
	/*if (l.xnor == 1) {
	backward_scale_cpu(l.x_norm, l.delta, l.batch, l.c, l.w*l.h, l.scale_updates);

	scale_bias(l.delta, l.scales, l.batch, l.c, l.h*l.w);

	mean_delta_cpu(l.delta, l.variance, l.batch, l.c, l.w*l.h, l.mean_delta);
	variance_delta_cpu(l.x, l.delta, l.mean, l.variance, l.batch, l.c, l.w*l.h, l.variance_delta);
	normalize_delta_cpu(l.x, l.mean, l.variance, l.mean_delta, l.variance_delta, l.batch, l.c, l.w*l.h, l.delta);
	return;
	}*/
...
{
	/*if (l.xnor) {
	copy_ongpu(l.inputs*l.batch, state.input, 1, l.x_gpu, 1);
	if (state.train) {
	#ifdef CUDNN
	float one = 1;
	float zero = 0;
	...
	normalize_gpu(state.input, l.rolling_mean_gpu, l.rolling_variance_gpu, l.batch, l.c, l.h*l.w);
	scale_bias_gpu(state.input, l.scales_gpu, l.batch, l.c, l.h*l.w);
	add_bias_gpu(state.input, l.biases_gpu, l.batch, l.c, l.w*l.h);
	}
	return;
	}*/
...
void backward_batchnorm_layer_gpu(layer l, network_state state)
{
	...
	cudnnBatchNormalizationBackward(cudnn_handle(),
		...
		l.variance_gpu);		// input (should be FP32)
								/*if (l.xnor)
								copy_ongpu(l.inputs*l.batch, l.x_norm_gpu, 1, l.delta_gpu, 1);
								else*/
